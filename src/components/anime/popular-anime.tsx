  ...
  useEffect(() => {
    const fetchPopular = async () => {
      try {
        ...
        const { getPopularAnime } = await import('@/lib/consumet/actions');
        const results = await getPopularAnime(formData);
        setPopular(results.slice(0, 10)); // Limit to 10 items
      } catch (err) {
    ...
    return (
      <Card className="p-6 border-destructive">
        <div className="space-y-2">
          <p className="text-destructive text-sm font-medium">Error loading popular anime:</p>
    ...
    <div className="space-y-4">
      ...
      {popular.length > 0 ? (
        ...
        <Card className="p-8 text-center">
          <p className="text-muted-foreground">No popular anime available</p>
        </Card>
      )}
    </div>
  );
