...
export function RecentEpisodesSection() {
  // Since Crunchyroll doesn't have recent episodes, we'll show recent anime instead
  const { data, loading, error } = useCrunchyrollSearch('recent', 1);

  if (loading) {
    return (
      <section className="space-y-6">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold">Recent Anime</h2>
        </div>
    ...
    <section className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button variant="ghost" asChild>
          <Link href="/anime/crunchyroll" className="flex items-center gap-1">
            Browse All
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {data.results.slice(0, 12).map((anime) => (
          <Link key={anime.id} href={`/anime/crunchyroll/${anime.id}`}>
                ...
                <CardContent className="flex-1 p-3 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    <div className="flex justify-between items-center mt-1">
                      <span>{anime.totalEpisodes ? `${anime.totalEpisodes} episodes` : 'Ongoing'}</span>
                      {anime.type && (
